...
所謂的「next step」，不一定要原子化到只剩下「執行一個 pure function 然後回傳結果」。當你把「tool calls」視為 model 輸出的 JSON，用來描述 deterministic code 應該做什麼時，你就能解鎖大量彈性。把這個觀念再與 [factor 8 own your control flow](https://github.com/humanlayer/12-factor-agents/blob/main/content/factor-08-own-your-control-flow.md) 搭配起來看。
...
[← 掌握你的 context window](https://github.com/humanlayer/12-factor-agents/blob/main/content/factor-03-own-your-context-window.md) | [統一 execution state →](https://github.com/humanlayer/12-factor-agents/blob/main/content/factor-05-unify-execution-state.md)
